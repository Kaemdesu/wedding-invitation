...
function Portrait(props: { person: Person; align: 'left' | 'right' }) {
  ...
  return (
    <motion.div variants={fadeUp} className={'flex flex-col items-center text-center ' + alignClass}>
      ...
      <h3 className="mt-6 font-heading text-fluid-2xl font-light italic text-gradient-gold md:mt-8">{person.name}</h3>
      <p className="mt-4 max-w-md font-sans text-fluid-base leading-relaxed text-cream/85">{person.bio}</p>
    </motion.div>
  )
}
...
export function CoupleSection() {
  return (
    <section className="relative px-6 py-24 safe-x md:py-32">
      ...
      <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={viewportDefaults} className="mx-auto grid max-w-6xl gap-16 md:grid-cols-2 md:gap-12 lg:gap-20">
